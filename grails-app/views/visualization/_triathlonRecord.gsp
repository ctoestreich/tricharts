...
<g:set var="keys" value='["${RaceCategoryType.Sprint}_${SegmentType.Swim}":"Sprint Swim","${RaceCategoryType.Sprint}_${SegmentType.Bike}":"Sprint Bike","${RaceCategoryType.Sprint}_${SegmentType.Run}":"Sprint Run"]'  />

<g:each in="${keys}" var="key">
    <div class="span2"><div id="${key.key}" class="personalBestDiv">
        <div class="btn-group-wrap" >
            <h3><a href="#" class="tooltips" rel="popover" title="${data?.get("'${key.key}'")?.raceResult?.race}" data-content="">${data?.get("'${key.key}'")?.pace}&nbsp;</a></h3>
            <h4>${key.value}</h4>
        </div>
    </div></div>
</g:each>
