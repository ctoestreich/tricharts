<div class="row-fluid">
  <div class="span4 clsResultBig"><a class="accordion-toggle" data-toggle="collapse" href="#result-collapse-${result?.id}">${result?.race}</a></div>
  <div class="span3 clsResultBig"><span class="accordion-toggle">${result?.race?.raceCategoryType}</span></div>
  <div class="span3 clsResultBig"><span class="accordion-toggle">${result?.date?.format("M/dd/yyyy")}</span></div>
  <div class="span2 clsResultBig"><span class="accordion-toggle"><tri:formatDuration duration="${result?.duration}"/></span></div>
</div>